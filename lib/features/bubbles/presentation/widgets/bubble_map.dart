    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        if (_packed.isEmpty || _lastSize != canvasSize) {
          // 🔧 CORREÇÃO: garante hierarquia física alinhada ao rank
          final orderedBubbles = [...widget.bubbles]
          ...
          child: ClipRect(
            child: AnimatedBuilder(
              ...
              builder: (context, _) {
                ...
                final sortedPacked = [..._packed]
                  ..sort((a, b) =>
                      b.bubble.rank.compareTo(a.bubble.rank));

                return Stack(
                  clipBehavior: Clip.none,
                  children: sortedPacked.map((p) {
                    ...
                    final dist =
                        (bubbleCenter - viewportCenter).distance;
                    final maxDist =
                        sqrt(vw * vw + vh * vh) * 0.55;

                    // fator 0 → centro | 1 → periferia
                    final depth =
                        (dist / maxDist).clamp(0.0, 1.0);

                    // escala perceptiva (centro mais forte)
                    final scale =
                        lerpDouble(1.28, 0.78, depth)!;

                    // opacidade perceptiva
                    final opacity =
                        lerpDouble(1.0, 0.35, depth)!;
